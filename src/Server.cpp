...
void sendPublishMessage(string publish_channel,string publish_message,int client_fd)
{
...
pair<string,bool> RespParser(istream &input, ClientState &curr_client) {
    ...
    if (server_role == "master") response_flag = true;
    else if (server_role == "slave") response_flag = false;
    string word;
    getline(input, word);
    ...
    if (server_role == "slave") replicationState.master_repl_offset += totalBytes;
    string command = data[0];
    transform(command.begin(), command.end(), command.begin(), ::tolower);
    cout << command << " " << totalBytes << endl;
    string output;

    if(command != "exec" && command != "discard" && command != "multi" && curr_client.clientQueueFlag == 1)
    {
    ...
    {
        if(curr_client.clientQueueFlag == 0) output = "-ERR EXEC without MULTI\r\n";
        else
        {
            curr_client.clientQueueFlag = false;
            if(curr_client.clientMultiQueue.size() == 0) output = "*0\r\n";
            else
            {
                string exec_output = "";
                ...
                {
                    cout << "here" << endl;
                    vector<string> queued_input = curr_client.clientMultiQueue.front();
                    stringstream queued_input_str = writeBulkStringStream(queued_input);
                    curr_client.clientMultiQueue.pop();
                    cout << "Executing" << endl;
                    auto [queued_output,queued_response_flag] = RespParser(queued_input_str,curr_client);
    ...
    else if(command == "discard")
    {
        ...
        {
            curr_client.clientQueueFlag = false;
            while(!curr_client.clientMultiQueue.empty()) curr_client.clientMultiQueue.pop();
            output = "+OK\r\n";
        }
    }
    else if(command == "subscribe")
    {
        ...
        for(int i =1; i < data.size();i++)
        {
            ...
            cout << "pubsub " << channel_name << pubsub_channels[channel_name].size() << endl;
        }
    ...
    else if(command == "unsubscribe")
    {
        ...
        if(data.size() == 1)
        {
            for (auto it = curr_client.channelSet.begin(); it != curr_client.channelSet.end(); ) {
                auto channel = *it; // Capture the channel before erasing
                pubsub_channels[channel].erase(curr_client.client_fd);
                it = curr_client.channelSet.erase(it); // Erase safely and move the iterator forward
        ...
        else
        {
            for(int i = 1; i < data.size();i++)
            {
                ...
                transform(unsubscribe_channel.begin(), unsubscribe_channel.end(), unsubscribe_channel.begin(), ::tolower);
    ...
    else if(command == "publish")
    {
        ...
        transform(publish_channel.begin(), publish_channel.end(), publish_channel.begin(), ::tolower);
     ...
     else if(command == "pubsub")
     {
        ...
        transform(pubsub_command.begin(), pubsub_command.end(), pubsub_command.begin(), ::tolower);
        ...
        if(pubsub_command == "numsub")
        {
            ...
            for(int i =2; i < data.size();i++)
            {
                string channel_name = data[i];
                transform(channel_name.begin(), channel_name.end(), channel_name.begin(), ::tolower);
                outputNumsub += ":" + to_string(pubsub_channels[channel_name].size()) + "\r\n";
            }
     ...
     else if(command == "punsubscribe")
    {
        ...
        if(data.size() == 1)
        {
            for (auto it = curr_client.patternSet.begin(); it != curr_client.patternSet.end(); ) {
                auto pattern = *it; // Capture the pattern before erasing
                pubsub_patterns[pattern].erase(curr_client.client_fd);
                it = curr_client.patternSet.erase(it); // Erase safely and move the iterator forward
        ...
        else
        {
            for(int i = 1; i < data.size();i++)
            {
                ...
                transform(unsubscribe_pattern.begin(), unsubscribe_pattern.end(), unsubscribe_pattern.begin(), ::tolower);
                pubsub_patterns[unsubscribe_pattern].erase(curr_client.client_fd);
    ...
    for(int fd=0; fd <= max_fd;fd++)
    {
       if (FD_ISSET(fd, &ready_fds)) { // Check if this fd is ready
            if (fd == server_fd) {
                ...
                client_map[new_client_fd] = initializeClientState(new_client_fd, curr_client);
                //cout << "initiliazed " << new_client_fd << "size " << client_map.size() << endl;
            }
            else
            {
              ...
              auto [response,response_flag] = RespParser(input,curr_client);
              cout << response << endl;
              if (response_flag) send(fd, response.c_str(), response.size(), 0); 
              sendReplicaACK();
            }
          }
        }
